...
export function QuizPage({ weaknessMode, onExitWeakness }: Props) {
  ...
  const newQuestion = useCallback(() => {
    if (weaknessMode) {
      ...
      setQuestion(cats.length > 0 ? generateWeaknessQuiz(cats) : generateQuiz('normal'));
    } else {
  ...
  const handleAnswered = useCallback((user: import('../utils/learningLog').UserAnswer) => {
    if (question) {
      recordQuizAnswer(question, user, weaknessMode ? 'weakness' : 'quiz', weaknessMode ? undefined : difficulty);
    }
